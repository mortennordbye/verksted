...
export default function ReviewOverlay({
  ...
  return (
    <Overlay
      label={`review ${label}`}
      onClose={onClose}
      className="h-full w-full max-w-[980px] overflow-hidden rounded-xl sm:h-[90dvh]"
    >
      <header className="flex flex-wrap items-center gap-x-3 gap-y-1.5 border-b border-line px-3.5 py-2.5 font-mono text-[12px]">
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <VerdictButton
            ...
            onClick={() => send({ verdict: review.verdict === "needs-work" ? null : "needs-work" })}
            className="text-wait"
          >
            ⚠ needs work
          </VerdictButton>
          <button
            onClick={onClose}
            aria-label="close"
            className="tap-sq flex items-center justify-center px-2 text-faint hover:text-text"
          >
