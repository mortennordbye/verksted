...
function IconLink({
  ...
  return (
    <Link
      ...
      title={title}
      aria-label={title}
      className="tap-sq flex flex-none items-center justify-center rounded-[7px] border border-line bg-surface px-2.5 py-1.5 text-muted hover:border-faint hover:text-text"
    >
      <svg
        viewBox="0 0 24 24"
        width="15"
  ...
  return (
    <header className="sticky top-0 z-20 flex flex-none items-center gap-3 border-b border-line bg-bg/90 px-[18px] py-2.5 pt-[max(10px,env(safe-area-inset-top))] backdrop-blur-md min-[800px]:py-3.5 min-[800px]:pt-[max(14px,env(safe-area-inset-top))]">
      ...
      <div className="ml-auto flex flex-none items-center gap-3">
        <IconLink to="/runs" title="inbox — what the schedules did">
